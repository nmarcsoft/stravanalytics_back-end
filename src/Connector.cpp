...
void ConnectionManager::execute()
{
    ...
    bool parsingSuccessful = Json::parseFromStream(readerBuilder, command_file, &command, &errors);
    ...
    {
        this->url = "https://www.strava.com/api/v3/activities"; // On veut récupérer une activitée
        if (command["activities"]["id"] != "") // On veut récupérer quelque chose sur une activités précise
        {
            this->url.append("/");
            this->url = this->url.append(command["activities"]["id"].asString());
            
            if (command["activities"]["comments"] != "") // On veut récupérer les commentaires
            {
    ...
    {
        this->url = "https://www.strava.com/api/v3/athlete"; // On veut récupérer une activitée
